...
// Retrieve JSON File Location from URL Ending ?=
...
console.log("Object.keys(recipeCollection)", Object.keys(recipeCollection));
...
function createMenueStringforClipboard(selectedRecipes, allFullRecipes, header) {
  date = new Date();
  console.log("selectedRecipes: ", selectedRecipes);
  // Hier rausnehmen und einfach als Header-Variable übergeben
  let menuString = header;
  //String: Overview of selected recipes
  menuString += selectedRecipes.join(", ").toUpperCase();
  // Add complete selected recipes
  let allFullRecipesArray = [];
  for (recipeName in allFullRecipes) {
      allFullRecipesArray.push(allFullRecipes[recipeName], );
      // Add all these full recipes, that are in the selectedRecipes Array
      // selectedRecipesFull.push(allFullRecipes.get(recipe));
    };
    console.log("allFullRecipesArray: ",allFullRecipesArray);
  
  // console.log("selectedRecipesFull", selectedRecipesFull);

  // menuString += JSON.stringify(allRecipes, "", 2);
    // recipeCollection.filter(recipe => recipes.indexOf(Object.keys(recipe)));
...
var vm = new Vue({
  ...
  methods: {
    ...
    generateMenuString: function() {
      let header = `Menüliste ab dem ${date.toLocaleDateString("de-DE", {
        weekday: "short",
        year: "numeric",
        month: "long",
        day: "numeric"
      })}\n`
      return createMenueStringforClipboard(this.selectedRecipes, this.recipes, header);
    },
    onError: function() {
